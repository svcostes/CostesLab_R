...
fe_plate_count_19C = length(fe_plate_nums_19C) #copy lines 169 and 170 for Si and Ar chunks for 19C
gamma_fe_plate_nums_19C = c(gamma_plate_nums_19C,fe_plate_nums_19C) #be sure to add Ar and Si plate nums to this line, refer to 18B chunk 
gamma_fe_plate_count_19C = length(gamma_fe_plate_nums_19C) #be sure to add Ar and Si plate nums to this line, refer to 18B chunk
...
gamma_table_19C=foci_gamma_first_19C #cop lines 175-177 for Si and Ar chunks, replace radiation with Ar and Si, refer to 18B chunk
...
} #copy lines 184-188 for Ar and Si, replace radiation with Ar and Si
...
ar_table = rbind(ar_table_18B, ar_table_19A) #be sure to add ar tables for 19C here, same for the rest of the conditions
...
colnames(Plate_Duplicate)[colnames(Plate_Duplicate)=="ï..Plate_number"]<-"Plate_number"
...
all_RIF_Raw = merge(all_RIF, well_meta, by = c("Run_set_well","Run_set"), all.x = TRUE)
```
