...
## In human samples! 
## Egle Cekanaviciute, USRA/NASA Ames, August 2019
## For run 19A: Set 1

## To be done: add Sets 2-4, compare with Run 18B, compare with baselines
...
```{r}
setwd("~/Desktop/Radiation_genomics/BNL_19A_DNA_damage") ## Change as necessary: this is the main directory
theme_set(theme_bw(base_size = 12)) ## For prettier graphs

## Get plate list for 19A
plate_list_19a = read.csv("19A_plate_list.csv", sep = ",", header = TRUE)

## All plate numbers for Si, Fe and Ar - specifically for this Set 1A from 19A!! Obviously change these as we include more Sets

ar_plate_nums = plate_list_19a$Plate_number[plate_list_19a$Rad_type == "Ar" & plate_list_19a$Sample_sets == "Set_1" & plate_list_19a$Duplicates == "A"]
ar_plate_count = length(ar_plate_nums)
si_plate_nums = plate_list_19a$Plate_number[plate_list_19a$Rad_type == "Si" & plate_list_19a$Sample_sets == "Set_1" & plate_list_19a$Duplicates == "A"]
si_plate_count = length(si_plate_nums)
fe_plate_nums = plate_list_19a$Plate_number[plate_list_19a$Rad_type == "Fe" & plate_list_19a$Sample_sets == "Set_1" & plate_list_19a$Duplicates == "A"]
fe_plate_count = length(fe_plate_nums)
gamma_plate_nums = plate_list_19a$Plate_number[plate_list_19a$Rad_type == "Gamma" & plate_list_19a$Sample_sets == "Set_1" & plate_list_19a$Duplicates == "A"]
gamma_plate_count = length(gamma_plate_nums)


all_plate_nums = c(si_plate_nums, fe_plate_nums, ar_plate_nums, gamma_plate_nums)

## To remove unprocessed plates - ONLY on 08/16/19 UNTIL the plates get processed!!!
unprocessed_plates =c(505,521,553,609)
si_plate_nums = si_plate_nums[!si_plate_nums %in% unprocessed_plates]
ar_plate_nums = ar_plate_nums[!ar_plate_nums %in% unprocessed_plates]
all_plate_nums = all_plate_nums[!all_plate_nums %in% unprocessed_plates]
## ONCE ALL PLATES GET PROCESSED, GET RID OF THE ABOVE CHUNK!!!
si_plate_count = length(si_plate_nums)
ar_plate_count = length(ar_plate_nums)
all_plate_count = length(all_plate_nums)
```

```{r}
## Read all metadata
subject_meta = read.csv("BNL_19A_sample_metadata_20190510_for_calc_20190527.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
subject_ids = unique(subject_meta$Sample_ID)
num_subj = length(subject_ids)
```


## 08/16/19: Si, Fe, Ar, Gamma separate
...
## Read first file
foci_si_first = data.frame(Plate = si_plate_nums[1], Plate_well = paste(si_plate_nums[1], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', si_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', si_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE))
...
for(ii in 2:si_plate_count){
  temp_filename = paste('BNL19A-ProcessedFiles-Set1A/average_well_P', si_plate_nums[ii],'.txt', sep="")
  temp_table = data.frame(Plate = si_plate_nums[ii], Plate_well = paste(si_plate_nums[ii], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', si_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', si_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE))
  si_table = rbind(si_table, temp_table)
}

## Now we will add plate metadata
si_table_w_plate_meta = merge(si_table, plate_list_19a, by.x = 'Plate', by.y = 'Plate_number', all.x = TRUE, all.y = FALSE)
...
si_avg = si_trunc ## Only because here we only have set 1A!
...
## Read first file
foci_fe_first = data.frame(Plate = fe_plate_nums[1], Plate_well = paste(fe_plate_nums[1], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', fe_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', fe_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE))
# Note that: new columns Plate and Plate_well are made. 
...
for(ii in 2:fe_plate_count){
  temp_filename = paste('BNL19A-ProcessedFiles-Set1A/average_well_P', fe_plate_nums[ii],'.txt', sep="")
  temp_table = data.frame(Plate = fe_plate_nums[ii], Plate_well = paste(fe_plate_nums[ii], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', fe_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', fe_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE))
  fe_table = rbind(fe_table, temp_table)
}

## Now we will add plate metadata
fe_table_w_plate_meta = merge(fe_table, plate_list_19a, by.x = 'Plate', by.y = 'Plate_number', all.x = TRUE, all.y = FALSE)
...
fe_avg = fe_trunc ## Only because here we only have set 1A!
...
## Initial analyses: demographics, comparison with different types of data, etc. 
...
# Now plotting everyone using individual samples
all_plot_dots = ggplot(fe_avg, aes(x = Fluence, y = avg_nfoci, col = Gender)) + geom_point() + geom_jitter()
facet(all_plot_dots, facet.by = 'Timepoint')
```
...
## Read first file
foci_ar_first = data.frame(Plate = ar_plate_nums[1], Plate_well = paste(ar_plate_nums[1], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', ar_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', ar_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE))
# Note that: new columns Plate and Plate_well are made. 
...
for(ii in 2:ar_plate_count){
  temp_filename = paste('BNL19A-ProcessedFiles-Set1A/average_well_P', ar_plate_nums[ii],'.txt', sep="")
  temp_table = data.frame(Plate = ar_plate_nums[ii], Plate_well = paste(ar_plate_nums[ii], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', ar_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', ar_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE))
  ar_table = rbind(ar_table, temp_table)
}

## Now we will add plate metadata
ar_table_w_plate_meta = merge(ar_table, plate_list_19a, by.x = 'Plate', by.y = 'Plate_number', all.x = TRUE, all.y = FALSE)
...
print("How many wells are left after filtering?")
print(c(nrow(ar_filt),"out of", nrow(ar_long), "which is", nrow(ar_filt)/nrow(ar_long)))
...
ar_avg = ar_trunc ## Only because here we only have set 1A!
...
# Check: did we keep at least 1 sample of all subjects after averaging and filtering
if (length(unique(ar_avg$Sample_ID)) == num_subj) {
  print("All subjects have been retained")
  } else {
    print("Entire subjects were lost during averaging and filtering!")
    print ("Original IDs:")
...
## OK, now we have the dataframe with all the ar data, after averaging all duplicates. What questions do we want to ask?
...
```{r}
## Read first file
foci_first = data.frame(Plate = gamma_plate_nums[1], Plate_well = paste(gamma_plate_nums[1], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', gamma_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', gamma_plate_nums[1],'.txt', sep=""), sep="\t", header = TRUE))
# Note that: new columns Plate and Plate_well are made. 

## Read all the other files and combine them into a single giant file with ALL the plates and ALL the wells
# This is for gamma only
gamma_table = foci_gamma_first # Not to lose the original first file just in case 

for(ii in 2:gamma_plate_count){
  temp_filename = paste('BNL19A-ProcessedFiles-Set1A/average_well_P', gamma_plate_nums[ii],'.txt', sep="")
  temp_table = data.frame(Plate = gamma_plate_nums[ii], Plate_well = paste(gamma_plate_nums[ii], read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', gamma_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE)$Well, sep = "_"), read.table(paste('BNL19A-ProcessedFiles-Set1A/average_well_P', gamma_plate_nums[ii],'.txt', sep=""), sep="\t", header = TRUE))
  gamma_table = rbind(gamma_table, temp_table)
}

## Now we will add plate metadata
gamma_table_w_plate_meta = merge(gamma_table, plate_list_19a, by.x = 'Plate', by.y = 'Plate_number', all.x = TRUE, all.y = FALSE)

## Now we will add well metadata

# Wells are different based on sample sets, so we need to make an extra column for Set_Well, and then add well metadata based on it.
gamma_table_w_plate_meta$Set_well = paste(gamma_table_w_plate_meta$Sample_sets, gamma_table_w_plate_meta$Well, sep = "_")
gamma_table_w_meta = merge(gamma_table_w_plate_meta, subject_meta, by.x = 'Set_well', by.y = 'Set_well')
...
# Visualize the data
hist(gamma_long$num_nuc) 

print("How many wells have been read at all?")
print(c(nrow(gamma_long)))
...
print("How many wells are left after filtering?")
print(c(nrow(gamma_filt),"out of", nrow(gamma_long), "which is", nrow(gamma_filt)/nrow(gamma_long)))
...
# Now we will need to average A and B duplicates
#gamma_avg = ddply(gamma_trunc, .(ID_Fluence_Timepoint, Sample_ID, Set, Rad_type, Dose_Fluence, Timepoint, Age, Gender, BMI, BMI_group), summarise, avg_nfoci = mean(avg_nfoci))


gamma_avg = gamma_trunc ## Only because here we only have set 1A!

print("How many samples do we have now?")
print(c(nrow(gamma_avg)))

# I don't like calling "Dose_Fluence" when we only have dose; also it is a factor instead of a number
...
# Also I want my BMI group to go from Normal to Overweight to Obese in outputs
gamma_avg$BMI_group = factor(gamma_avg$BMI_group, levels = c("Normal", "Overweight", "Obese")) 

# Check: did we keep at least 1 sample of all subjects after averaging and filtering
if (length(unique(gamma_avg$Sample_ID)) == num_subj) {
  print("All subjects have been retained")
  } else {
    print("Entire subjects were lost during averaging and filtering!")
    print ("Original IDs:")
    print(subject_ids)
    print("New IDs:")
    print(unique(gamma_avg$Sample_ID))
  }

## Initial analyses: demographics, comparison with different types of data, etc. 

## OK, now we have the dataframe with all the gamma data, after averaging all duplicates. What questions do we want to ask?

# First, let's look at it very quickly by dose at 4h and 24h
quick_sum = ddply(gamma_avg, .(Timepoint, Dose), summarise, mean = mean(avg_nfoci), sd = sd(avg_nfoci))
quick_sum

# Now plotting everyone using individual samples
all_plot_dots = ggplot(gamma_avg, aes(x = Dose, y = avg_nfoci, col = Gender)) + geom_point() + geom_jitter()
facet(all_plot_dots, facet.by = 'Timepoint')
```
...
## Everything below from here is old!
