...
animal_ID <- read.table('/Users/epariset/Desktop/Sylvain/IDvsSpecies.csv',sep=",",header=TRUE)
...
# Reads first file
foci <- read.table(paste('/Users/epariset/Desktop/Sylvain/average_well/average_well_P',list_group[1],'.txt',sep=""),sep="\t",header=TRUE)
...
factors_res <- read.table('/Users/epariset/Desktop/Sylvain/Factors.csv',sep=",",header=TRUE) # defines value of residual damage (percentage of value at 48h): diferent for each strain based on fit optimization
...
for (i_strain in 1:num_strain) {
  for (i_dose in 1:3) {
    ...
    if (sum(keep_ind)>0) { # only add if the condition is met
      ...
      mean_foci_strain_xray[keep_ind,]$mean_sub = mean_foci_strain_xray[keep_ind,]$mean - factors_res$Residual_factor[i_strain] * mean_foci_strain_xray[keep_ind2,]$mean
    }
  }
}
