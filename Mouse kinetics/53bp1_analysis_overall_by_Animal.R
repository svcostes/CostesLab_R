    ...
    for (i_dup in 1:2) {
      ...
      if (i_let < num_let) { # First 3 data sets are high-LET, dose dependence is linear as foci reflects # of tracks
        ...
        if (i_let == 1 & i_time == 1 & i_dup == 1) {
          ...
          colnames(list_fits) = c(paste(let_list[i_let],time_list[i_time],"Bgd"),paste(let_list[i_let],time_list[i_time],"FociPerGy"),paste(let_list[i_let],time_list[i_time],"duplicate"))
        } else{
          coef_fits = rbind(coef_fits,cbind(tmp_fits,time_list[i_time],let_list[i_let],rownames(tmp_fits)))
          colnames(tmp_fits) = c(paste(let_list[i_let],time_list[i_time],"Bgd"),paste(let_list[i_let],time_list[i_time],"FociPerGy"),paste(let_list[i_let],time_list[i_time],"duplicate"))
        ...
        if (i_time == 1) { # exclude 4 Gy for 4 hour time point
          keep_ind = (foci$Radiation.Type==let_list[i_let] & foci$num_nuc>20 & foci$Radiation<=1 & foci$Radiation.Time == time_list[i_time] & foci$duplicate==i_dup) # Select data for given LET and time
          } 
...
colnames(coef_fits) = c("Bgd","FociPerGy","duplicate","Time","let","UserID")
...
# Check how fit duplicated
ind_dup1 = coef_fits[,"duplicate"]==1
ind_dup2 = coef_fits[,"duplicate"]==2
coef_fits_dup = merge(coef_fits[ind_dup1,],coef_fits[ind_dup2,],by=c("Time","let","UserID"))
keep_ind = coef_fits_dup$let !="Si 350 MeV/n"
qplot(Bgd.x, Bgd.y, data=coef_fits_dup[keep_ind,], size=I(2),
      facets= Time~., color=factor(let),
      xlab="Duplicate 1", ylab="Duplicate 2") +
  geom_smooth(method = "lm", se = TRUE)

qplot(FociPerGy.x, FociPerGy.y, data=coef_fits_dup[keep_ind,], size=I(2),
      facets= Time~., color=factor(let),
      xlab="Duplicate 1", ylab="Duplicate 2") +
  geom_smooth(method = "lm", se = TRUE)
...
avg_nfoci <- ddply(foci[keep_ind,], c("UserID", "Radiation.Type", "hour","Strain"), summarise,
                              mean = mean(avg_nfoci, na.rm=TRUE))
colnames(avg_nfoci)= c("UserID","Radiation.Type","hour","Strain","normal_Bgd")
coef_fits = merge(coef_fits,avg_nfoci,by.y=c('UserID','hour','Radiation.Type'), 
                  by.x=c('UserID','hour','let'),all=TRUE)
coef_fits = merge(coef_fits,animal_ID,by.x='UserID',by.y='Exp_ID')
coef_fits = coef_fits[,c(seq(1,6),seq(9,12))]

coef_fits_no_dup = ddply(coef_fits, c("UserID", "let", "hour","Strain.y"), summarise,
                         intercept = mean(Bgd, na.rm=TRUE),
                         Bgd = mean(normal_Bgd, na.rm=TRUE),
                         FociPerGy = mean(FociPerGy, na.rm=TRUE))
                         
write.csv(coef_fits_no_dup,'phenotypes_by_UserID.csv')

# add background and fitted coefficients to foci data.frame
foci <- merge(foci, coef_fits[,seq(1,7)], by.x=c('UserID','hour','Radiation.Type','duplicate'), 
              by.y=c('UserID','hour','let','duplicate'))
...
foci_test = foci[keep_ind,c("UserID", "Radiation.Type", "hour","duplicate","nfoci_bgdsub","norm_nfoci","avg_nfoci","let","Radiation")]
mean_foci_UserID_let <- ddply(foci_test, c("UserID", "Radiation.Type", "hour","let"), summarise,
...
foci_test = foci[keep_ind,c("UserID", "Radiation.Type", "hour","duplicate","nfoci_bgdsub","norm_nfoci","avg_nfoci","let","Radiation")]
mean_foci_UserID_xray <- ddply(foci_test, c("UserID", "Radiation.Type", "hour","Radiation"), summarise,
...
keep_ind = mean_foci_UserID_let$hour == 4
# compute slope of saturation between two RIF/um data point for LET
fits <- lmList(mean ~ poly(let, 1, raw = TRUE) | UserID, data=mean_foci_UserID_let[keep_ind,])
saturation_fit = coef(fits)
# Compute ratio of slope between low and high let
keep_ind = mean_foci_UserID_let$hour == 4 & mean_foci_UserID_let$let==let_val[2]
saturation_fit$ratio = saturation_fit[,2]/mean_foci_UserID_let[keep_ind,"mean"]*let_val[2]
saturation_fit$UserID = rownames(saturation_fit)
# compute slope and saturation for Xray
...
high_slope = (mean_foci_UserID_xray$mean[keep_ind2]-mean_foci_UserID_xray$mean[keep_ind1])/3
low_slope = (mean_foci_UserID_xray$mean[keep_ind]/0.1 + mean_foci_UserID_xray$mean[keep_ind1])/2
xray_slope = mean_foci_UserID_xray[keep_ind,c(1,3)] # initialize this df to only one value per animal
xray_slope$low_slope = low_slope
xray_slope$ratio = low_slope/high_slope
merge(saturation_fit,xray_slope[,c(1,3,4)],by = 'UserID')
