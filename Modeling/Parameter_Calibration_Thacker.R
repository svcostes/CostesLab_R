...
# S.V. Costes, August 2019, NASA - modified by E. Pariset in September 2020
...
num_LET = num_E
LET = 110 # Which LET is to be used to minimize residual of all fits. List of LETs with mutation data, for V79: 1, 20, 50, 90, 110, 200, 470 for HF19: 1, 20, 28, 50, 70, 90, 110, 160, 200, 470
  ...
  i_E = optim_index
    ...
    for (i_D in 1:n_dose) {
      ...
      if (Thack_data[i_E,'Z']==2 & Thack_data[i_E,'LET..keV.um.'] < 50) { # Helium simulation and low LET
        file_path = paste('data/RIFSize_He/Size ',dom_size,' microns/RIFs_He_',Thack_data[i_E,4],'_MeV_',D_ar_lowLET[i_D],'_Gy.dat',sep="")
      }
      if (Thack_data[i_E,'Z']==2 & Thack_data[i_E,'LET..keV.um.'] >= 50) { # Helium simulation and high LET
        file_path = paste('data/RIFSize_He/Size ',dom_size,' microns/RIFs_He_',Thack_data[i_E,4],'_MeV_',D_ar_highLET[i_D],'_Gy.dat',sep="")
      }
      if (Thack_data[i_E,'Z']==5) { # Boron simulation
        file_path = paste('data/RIFSize_B/Size ',dom_size,' microns/RIFs_B_',Thack_data[i_E,4],'_MeV_',D_ar_highLET[i_D],'_Gy.dat',sep="")
      }      
