...
tif_dir = 'D:\G-SPEED\BNL19A\TIFF-SetA'; % Where tiff from Zeiss CD7 have been put
...
out_dir = 'D:\G-SPEED\BNL19A\Processed-SetA'; % Directory where to save cropped images
...
while read_flag
    ...
    nuc_img = dip_image(imread(sprintf('%s\\%s\\%s_s%sz%s_%s.tif',tif_dir,file_name,file_name,int2str_format(i_cnt,num_digit),int2str_format(num_z/2+1,2),dapi_str)));
