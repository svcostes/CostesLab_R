...
function lab = relabel_v2(varargin)
...
if L(end)~=length(L)        % only do anything if the labels IDs aren't continuous.
   ...
   if length(N)<3
       N = [N 1 1]; %lut crashes if N is made of one or two values only
   end
 %  N = dip_image([0,N],datatype(lab));
   lab = lut(lab,[0,N]);        % do the look-up thing.
end
lab = dip_image(uint16(lab));
