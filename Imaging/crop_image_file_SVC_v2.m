...
warning('off','all'); %This is to remove the annoying warning about writing 16 bit tiff
...
for i_field =1:num_field % Start reading each field and crop them; AB --- changed 49 to 1
    ...
    fprintf('Nuclear segmentation done. Selecting round cells only. Cropping Nuclei.\n');
    for i_nuc = 1:num_cells
        if (msr(i_nuc).P2A < P2A_min && msr(i_nuc).size>size_cutoff) % Only keep non deformed cells
            ...
            if (nuc_cnt == 1)
                coordinates_array = crop_coordinates;
            else
                coordinates_array = [coordinates_array;crop_coordinates];
            end
            nuc_cnt = nuc_cnt+1;
        end
    end
    fprintf('Kept %d out of %d initial cells\n',nuc_cnt-1,num_cells);
    num_cells = nuc_cnt-1;
    if num_cells == 0
        ...
        for i_ch = 1:num_ch
            fprintf('\nReading, cropping and saving cropped channel image %s\n',ch_name{i_ch});
            ...
            for i_nuc = 1:num_cells
                if size(ch_img,3) == 1
                    ch_img_cropped = ch_img(coordinates_array(i_nuc*2-1,1):coordinates_array(i_nuc*2-1,2),coordinates_array(i_nuc*2,1):coordinates_array(i_nuc*2,2)); % using 5 pixels to allow shift correction after acquisition , in case
                else
                    ch_img_cropped = ch_img(coordinates_array(i_nuc*2-1,1):coordinates_array(i_nuc*2-1,2),coordinates_array(i_nuc*2,1):coordinates_array(i_nuc*2,2),0:end); % using 5 pixels to allow shift correction after acquisition , in case
                end
            ...
            if size(ch_img,3) == 1
                tmp_col = overlay(squeeze(stretch(clip(ch_img,max_ch(i_ch),min_ch(i_ch)))),nuc_contour,[255,0,0]);
            else
        ...
        fprintf('\nSaving statistics in local variable\n');
