...
tif_dir = 'G:\BNL19A_Exported_TIFFs'; % Where tiff from Zeiss CD7 have been put
...
plate_name = 'P4511'; % This will name the new directory with cropped images and analysis.
file_name = '451_Gamma_1_4h_Set2_A';
...
num_img = 3; % number of full images per well
...
while read_flag
    ...
    if sub_well > num_img % we are done with this well, moving to next one. Renaming well.
        ...
        if abc==true
            fprintf('This has less than 100')
        end
        fprintf('Number of cell counted for well %s: %d\n',well_name,cell_count);
        fprintf(log_file,'Number of cell counted for well %s: %d\n',well_name,cell_count);
        cell_count = 0;
