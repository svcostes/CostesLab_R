...
% concatenate_well(nplate,ImageDir)
...
function concatenate_well_Ames(nplate,ImageDir)
...
if ischar(nplate) % allow nplate to be a string with current dir to be analyzed
    ...
    cur_dir = [ImageDir nplate '\'];
else
    plate_name=sprintf('P%d',nplate);
    cur_dir_d =dir([ImageDir plate_name '*']);% directory
    
    if length(cur_dir_d) ==1
        cur_dir= [ImageDir cur_dir_d.name '\'];
    elseif length(cur_dir_d) > 1
        ...
        cur_dir= [ImageDir cur_dir_d(keep_ind(end-k)).name '\'];
    else
...
for i_well = 3:num_well %ignore . and .. files.
    try
