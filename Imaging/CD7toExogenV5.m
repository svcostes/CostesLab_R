...
dip_initialise;
ASIDir = 'D:\Users\zeiss\Documents\MATLAB\ASI\'; % Tiger direct PC
addpath(ASIDir);
addpath([ASIDir, '\work']);
...
tif_dir = '\\Desktop-67r2rld\g-speed\BNL19A\TIFF-SetB\2019-09-18'; % Where tiff from Zeiss CD7 have been put
fitc_str = 'AF488';  % Replace to the right suffix for Dapi images
dapi_str = 'DAPI';  % Replace with the right suffix for FITC images
out_dir = 'G:\BNL19_cropped'; % Directory where to save cropped images
plate_name = 'P468'; % This will name the new directory with cropped images and analysis.
file_name = '468_Gamma_0.1_24h_Set2_B'
...
while read_flag
    ...
    dapi_name = sprintf('%s\\%s\\%s_S%s(P*)_Z%s_C01(%s)_M0000_ORG.tif',...
        tif_dir,file_name,file_name,int2str_format(i_cnt,num_digit),int2str_format((num_z-1)/2,num_digit),dapi_str);
    dapi_read = dir(dapi_name);
    if length(dapi_read)>1
        fprintf('Problem reading file. More than one image for given well and position\nString search used:%s\n',...
            dapi_name);
        break;
    end
    ...
    while (i_zc<=num_z-1)
        try
            fitc_name = sprintf('%s\\%s\\%s_S%s(P*)_Z%s_C00(%s)_M0000_ORG.tif',...
                tif_dir,file_name,file_name,int2str_format(i_cnt,num_digit),int2str_format(i_zc,num_digit),fitc_str);
