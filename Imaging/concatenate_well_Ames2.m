...
% averaged strings
% Well # Process_dir
fop_outf = fullfile(plate_dir, ['average_well_test' plate_name '.txt'])
fop = fopen(fop_outf,'w'); % Average file for all wells
fcop = fopen(fullfile(plate_dir, ['concat_plate_test' plate_name '.txt']),'w');% Concatenated output
...
for i_well = 1:num_well
    ...
    if num_process == 0
        ...
        for i_nuc = 1:num_nuc
            fprintf(fcop,'%s\t%s\t%s\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n',datat{2}{i_nuc},list_well(i_well).name,well_dirs(keep_ind(end)).name,...
                datat{3}(i_nuc),datat{5}(i_nuc),datat{6}(i_nuc),datat{7}(i_nuc),datat{8}(i_nuc),datat{9}(i_nuc), datat{10}(i_nuc),datat{11}(i_nuc));
        end
        p2a = datat{7};
        nfoci = datat{8};
        spot_mean = datat{9};
