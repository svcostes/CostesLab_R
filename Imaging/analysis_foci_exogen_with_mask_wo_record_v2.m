function analysis_foci_exogen_with_mask_wo_record_v2(target_dir, single_nuc_flag, params, chan_str, PRINT_FLAG )
...
% April 2023 - Edited code to make it more compatible with new load_params
% function
...
try % change from textread to import to be able to read manual qc_file. Added another line to turn sub_list to 3 digits string 7/24/2019
    ...
    idata = importdata([plate_dir '\qc_file.txt'],'\t',0);
    ...
    try % This will write a log if image failed to process
        ...
        if ~(qc_pass) % If we can keep the field or the field was not QC, read images for the field
            ...
            try
                ...
                fprintf('could not find:%s\n',[target_dir file_list{i}(1:end-8) chan_str '.ics']);
                i = i+1;
                continue;
            end
