function analysis_foci_exogen_with_mask_wo_record(target_dir, single_nuc_flag, params, chan_str, PRINT_FLAG )
...
[file_list, params] = update_list(target_dir, slash_str,params);
    ...
    try % This will write a log if image failed to process
        ...
        try
            fitc_img = readim([target_dir file_list{i}(1:end-8) chan_str '.ics']);
        catch
...
%         end
        ...
        % This piece of code is to check what metrics correlate between a
        % cell in focus and Psnr, and other values
%         dipshow(overlay(stretch(nuc2D),edge,[255 255 0]));
%         crop_mask = berosion(mask2D>0,5);
%         fprintf('Image %d: \nmax gaussf sub: %5.2f, psnr: %5.2f, rstd: %5.2f \n', get(gcf,'Number'),...
%         percentile(abs(nuc2D-gaussf(nuc2D,10))/nuc2D,90),...
%         psnr(nuc2D), std(nuc2D(crop_mask))/mean(nuc2D(crop_mask)));
