...
% plate parameters
parameter_file = 'C:\Users\genuser\Documents\MATLAB\ASI\work\Imaging\human_blood_params.txt';
chan_str = 'FITC';
params = load_params(parameter_file);
dir_num = [381]; %[updated on 5/09/19 by EP];
%dir_num = importdata('\\GENUSER-PC\Images\notProcessed.csv')
%csvwrite('\\GENUSER-PC\Images\processed',dir_num)

for i_plate = dir_num
    plate_name = sprintf('P%d',i_plate);
    %target_dir_list = dir(['D:\' plate_name '*']);
    target_dir_list = dir(['K:\' plate_name '*']);
    ...
    for i_dir = 1:num_plate_dir
        ...
        if target_dir_list(i_dir).isdir
            ...
            target_dir =  ['K:\' target_dir_list(i_dir).name '\'];
            ...
            for i_list = num_list
                try
                    ...
                    if ~isempty(process_file) % if more than one directory, only keep the good one
                        ...
                        if flag_good_dir % if still true, then, no directory had good results, rerun analysis
                            analysis_foci_exogen_with_mask_wo_record([target_dir file_list{i_list}], 1, params, chan_str, 1);
                        end
                    else % if here, that means there is no process directory. Need to run analysis
                        analysis_foci_exogen_with_mask_wo_record([target_dir file_list{i_list}], 1, params, chan_str, 1);
                    end
                catch
             ...
             concatenate_well_Ames(i_plate,'K:\');
             fImaged = fopen('K:\processed.csv','a')
             fprintf(fImaged,'%d\n',i_plate)
             fclose(fImaged)
        end
    end
