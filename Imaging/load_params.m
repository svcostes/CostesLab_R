...
try
    ...
    while i < length(param_import)
        switch(param_import{i})
            ...
            case {'main_spot'}
                params.chan_str = param_import{i+1};
            case {'num_ch'} % Special case. This assumes the next lines will have the full name of the channel, followed by the short on, followed by min stretch and max stretch
                params.num_ch = str2num(param_import{i+1});
                params.short_name = cell(1,params.num_ch);
                params.long_name = cell(1,params.num_ch);
                params.min_stretch = zeros(1,params.num_ch);
                params.max_stretch = zeros(1,params.num_ch);
                i=i+2;
                for i_ch = 1:params.num_ch
                    params.short_name{i_ch} = param_import{i};
                    if matches(params.short_name{i_ch},params.chan_str) % find which channel # is the spot channel
                        params.spot_index = i_ch;
                    end
                    i= i + 1;
                end
                for i_ch = 1:params.num_ch
                    params.long_name{i_ch} = param_import{i};
                    i= i + 1;
                end
                for i_ch = 1:params.num_ch
                    params.min_stretch(1,i_ch) = str2num(param_import{i});
                    i= i + 1;
                end
                for i_ch = 1:params.num_ch
                    params.max_stretch(1,i_ch) = str2num(param_import{i});
                    i= i + 1;
                end
                params.fitc_min_stretch = params.min_stretch(1,params.spot_index);
                params.fitc_max_stretch = params.max_stretch(1,params.spot_index);
                i = i - 2;
            case {'measurement'}
            ...
            otherwise
                if ~isempty(str2num(param_import{i+1}))
                    params = feval(@setfield,params,param_import{i},str2num(param_import{i+1}));
                else
                    params = feval(@setfield,params,param_import{i},param_import{i+1});
                end
        end
        i = i + 2;
    end
